...
<?php
function login($username, $password, $aqua){
	// Menggunakan perintah prepared statement untuk menghindari SQL injection
	if($stmt = $aqua->prepare("SELECT id, password FROM users WHERE username = ?")){
...
function cek_login($aqua){
	...
	if(isset($_SESSION['id'], $_SESSION['password_login'])){
		...
		if($stmt = $aqua->prepare("SELECT password FROM users WHERE id = ? LIMIT 1")){
