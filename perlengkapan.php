...
<?php
	...
	include 'config.php';
 	$perhalaman = 6;
 	$data = mysqli_query($aqua,"SELECT * FROM perlengkapan");
...
<body background="back.jpg" style="">
	...
	<div class="container">
		<div class="row">
			...
			$data ="SELECT * FROM perlengkapan LIMIT $awaldata, $perhalaman";
			...
			<div class="col s12 m4 l4">
				<div class="card">
					...
					<?php echo'	<img src="hardscape/'.$row['fotoperlengkapan'].'" class="activator responsive-img" alt="" />'; ?>
					</div>
					<div class="card-content">
					<?php echo'	<span class="card-title activator">'.$row['NamaPerlengkapan'].'<i class="material-icons right">keyboard_arrow_up</i></span>'; ?>
					</div>
					<div class="card-reveal">
					<?php echo'	<div class="card-title">'.$row['NamaPerlengkapan'].'<i class="material-icons right">close</i></div>';?>
					<?php echo'<p>'.$row['FungsiPerlengkapan'].'</p>';?>
					</div>
				</div>
			</div>
			<?php
		...
		<!-- Paging -->  
          <ul class="pagination white">
            <?php if( $halamanaktif > 1) : ?>
              <li class="waves-effect"><a href="perlengkapan.php?halaman=<?php echo $halamanaktif - 1; ?>"><i class="material-icons">skip_previous</i></a></li>
            <?php endif; ?>

            <?php for( $i = 1; $i <= $jumhalaman; $i++) : ?>
                <?php if( $i == $halamanaktif) : ?>
                  <li class="active lime accent-2"><a href="perlengkapan.php?halaman=<?php echo $i ?>"><?php echo $i; ?></a></li>
                <?php else : ?>
                  <li class="waves-effect"><a href="perlengkapan.php?halaman=<?php echo $i ?>"><?php echo $i; ?></a></li>
                <?php endif; ?>
            <?php endfor; ?>

            <?php if( $halamanaktif < $jumhalaman) : ?>
              <li class="waves-effect"><a href="perlengkapan.php?halaman=<?php echo $halamanaktif + 1; ?>"><i class="material-icons">skip_next</i></a></li>
            <?php endif; ?>
          </ul>
        <!-- Paging -->
